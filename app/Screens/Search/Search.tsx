...
const Search = ({ navigation, route }: SearchScreenProps) => {
    ...
    const { propertyTypeId, propertyTypeName } = route.params ?? {};

    const [searchQuery, setSearchQuery] = React.useState('');
    const [selected, setselected] = React.useState<typeof categories>([]);

    const togglecategories = (option: (typeof categories)[0]) => {
        if (selected.some((s: any) => s?.id === option?.id)) {
            setselected(selected.filter((s: any) => s?.id !== option?.id));
        } else {
            setselected([...selected, option]);
        }
    };

    const goToResults = (searchText?: string, cityId?: number) => {
        const filters: Record<string, unknown> = {};
        if (searchText?.trim()) filters.search = searchText.trim();
        if (cityId != null && !isNaN(cityId)) filters.city_id = cityId;
        if (propertyTypeId != null && propertyTypeId !== 0) filters.property_type_id = propertyTypeId;
        navigation.navigate('Search_List', { filters: Object.keys(filters).length ? filters : undefined });
    };
                ...
                <View style={[GlobalStyleSheet.container, { padding: 20, flex: 1 }]}>
                    {propertyTypeName ? (
                        <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 12, gap: 8 }}>
                            ...
                            >
                                <Text style={[FONTS.BodyS, FONTS.fontSemiBold, { color: COLORS.white }]}>
                                    {propertyTypeName}
                                </Text>
                            </View>
                            <Text style={[FONTS.BodyXS, { color: colors.gray60 }]}>selected</Text>
                        </View>
                    ) : null}
                    <View style={{ marginBottom: 5 }}>
                        <TextInput
                            ...
                            placeholder='Search property, location, city...'
                    ...
                    <View style={{ paddingVertical: 10 }}>
                        ...
                        <View style={{ flexDirection: "row", flexWrap: 'wrap', gap: 7 }}>
                            {categories.map((item: (typeof categories)[0]) => {
                ...
                <View style={[GlobalStyleSheet.container, { paddingHorizontal: 20, paddingBottom: 30 }]}>
                    <Button
                        ...
                        onPress={() => navigation.navigate('SearchArea', {
                            ...
                            propertyTypeId: propertyTypeId != null && propertyTypeId !== 0 ? propertyTypeId : undefined,
                            propertyTypeName: propertyTypeName || undefined,
                        })}
                    />
                </View>
            </Animated.View>
            <View style={[GlobalStyleSheet.container, { paddingHorizontal: 20, paddingBottom: 20, paddingTop: 10 }]}>
                <Button
                    title='See results'
                    btnRounded
                    color={theme.dark ? COLORS.darkwhite : COLORS.white}
                    text={theme.dark ? '#9654F4' : COLORS.primary}
                    outline
                    onPress={() => goToResults(
                        searchQuery.trim() || undefined,
                        selected?.[0] && typeof selected[0] === 'object' && 'id' in selected[0] ? Number((selected[0] as any).id) : undefined
                    )}
                />
            </View>
        </SafeAreaView>
